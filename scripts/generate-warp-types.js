...
function inferType(value, key) {
  ...
  if (value.startsWith('transform:')) {
    ...
    if (code.includes('.toFixed(') || code.includes('.toLocaleString(') || code.includes('`') || (code.includes(' + \'') || code.includes(' + "'))) return 'string';
    if (code.includes('|| 0') || code.includes('Number(') || code.includes(' / ') || code.includes(' * ') || code.includes(' + ') || code.includes(' - ')) {
        // Exclude string concatenation
        if (!code.includes(' + \'') && !code.includes(' + "')) {
            return 'number';
        }
    }
    if (code.includes('|| \'') || code.includes('|| "')) return 'string';
    if (code.includes('|| false') || code.includes('|| true')) return 'boolean';
    if (code.includes('.map(') || code.includes('|| []')) {
        if (code.includes('.hash') || code.includes('.name') || code.includes('.address')) return 'string[]';
        return 'any[]';
    }
    if (code.includes('BigInt(')) return 'string';
    if (code.includes('===') || code.includes('!==') || code.includes('>') || code.includes('<') || code.includes('&&') || code.includes('||')) {
        if (code.includes('true') || code.includes('false') || key.startsWith('IS_') || key.startsWith('HAS_')) {
            return 'boolean';
        }
    }
  }
  ...
  if (key.endsWith('_EGLD') || key.endsWith('_AMOUNT') || key.endsWith('_PRICE') || key.endsWith('_LIMIT') || key.endsWith('_USED') || key === 'NONCE' || key === 'TIMESTAMP' || key === 'ROUND') return 'number';
