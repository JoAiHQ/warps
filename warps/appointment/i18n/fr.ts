import { en } from './en'

export const fr: typeof en = {
  book: {
    title: 'Prendre un rendez-vous',
    pickDate: 'Choisir une date',
    pickSlot: 'Choisir un horaire',
    noSlots: 'Aucun créneau disponible ce jour. Essaie un autre jour.',
    loadingSlots: 'Vérification des disponibilités...',
    yourDetails: 'Tes coordonnées',
    namePlaceholder: 'Ton nom',
    emailPlaceholder: 'Ton e-mail',
    purposePlaceholder: 'De quoi s\'agit-il ? (optionnel)',
    notesPlaceholder: 'Autre chose à savoir ? (optionnel)',
    confirm: 'Confirmer la réservation',
    booking: 'Réservation en cours...',
    confirmed: 'Réservé !',
    confirmedMessage: 'Ton rendez-vous est confirmé.',
    backToSlots: 'Retour',
    today: 'Aujourd\'hui',
    tomorrow: 'Demain',
    nameRequired: 'Le nom est obligatoire.',
    timeZone: 'Fuseau horaire',
    cancelAppointment: 'Annuler le rendez-vous',
    cancelling: 'Annulation en cours...',
    cancelled: 'Rendez-vous annulé',
    cancelledMessage: 'Ton rendez-vous a été annulé.',
    cancelConfirm: 'Annuler ce rendez-vous ?',
    cancelConfirmYes: 'Oui, annuler',
    cancelConfirmNo: 'Garder',
    cancelReasonLabel: 'Motif (optionnel)',
    cancelReasonPlaceholder: 'Pourquoi annules-tu ?',
    slotsError: 'Impossible de charger les créneaux disponibles. Réessaie.',
    bookingError: 'Impossible de réserver ce créneau. Choisis une autre heure.',
    cancelError: 'Impossible d\'annuler ton rendez-vous. Réessaie.',
    tryAgain: 'Réessayer',
    copyLink: 'Partager ce lien',
    copied: 'Copié !',
    serviceDuration: 'min',
    bookService: 'Réserver',
    noSpecificService: 'Aucun service en particulier',
  },
  confirm: {
    confirmed: 'Confirmé !',
    confirmedMessage: 'Ton rendez-vous est confirmé.',
    notFound: 'Rendez-vous introuvable',
    notFoundMessage: 'Ce rendez-vous a peut-être été supprimé ou le lien est invalide.',
    cancelAppointment: 'Annuler le rendez-vous',
    cancelling: 'Annulation...',
    cancelled: 'Rendez-vous annulé',
    cancelledMessage: 'Ce rendez-vous a été annulé.',
    cancelConfirm: 'Annuler ce rendez-vous ?',
    cancelConfirmYes: 'Oui, annuler',
    cancelConfirmNo: 'Garder',
    cancelReasonLabel: 'Motif (optionnel)',
    cancelReasonPlaceholder: 'Pourquoi annules-tu ?',
    cancelError: 'Impossible d\'annuler. Réessaie.',
    joinTeams: 'Rejoindre sur Microsoft Teams',
    joinGoogleMeet: 'Rejoindre sur Google Meet',
    joinZoom: 'Rejoindre sur Zoom',
    joinMeeting: 'Rejoindre la réunion',
  },
  configure: {
    title: 'Paramètres de rendez-vous',
    officeHours: 'Horaires d\'ouverture',
    closed: 'Fermé',
    bookingRules: 'Règles de réservation',
    save: 'Enregistrer',
    saved: 'Enregistré',
    saving: 'Enregistrement...',
    fields: {
      slotInterval: 'Intervalle de créneaux (min)',
      bufferAfter: 'Tampon après (min)',
      minNotice: 'Préavis minimum (min)',
      maxDaysAhead: 'Jours à l\'avance max',
      conferenceEnabled: 'Lien de réunion en ligne',
      serviceSelectionEnabled: 'Permettre la sélection de service',
      paymentEnabled: 'Paiement marketplace requis',
      serviceDurationUnit: 'min',
    },
    days: {
      monday: 'Lun',
      tuesday: 'Mar',
      wednesday: 'Mer',
      thursday: 'Jeu',
      friday: 'Ven',
      saturday: 'Sam',
      sunday: 'Dim',
    },
  },
  policyGet: {
    title: 'Politique de rendez-vous',
    empty: 'Aucune politique de rendez-vous configurée pour le moment.',
    active: 'Active',
    inactive: 'Inactive',
    officeHours: 'Horaires d\'ouverture',
    noHours: 'Aucun horaire d\'ouverture défini.',
    bookingRules: 'Règles de réservation',
    details: 'Détails',
    editSettings: 'Modifier les paramètres',
    yes: 'Oui',
    no: 'Non',
    syncEnabled: 'Synchronisé avec les horaires du lieu',
    syncDisabled: 'Non synchronisé',
    syncError: 'Erreur de sync',
    fields: {
      slotInterval: 'Intervalle de créneaux',
      bufferAfter: 'Tampon après',
      minNotice: 'Préavis minimum',
      maxDaysAhead: 'Jours à l\'avance max',
      capacity: 'Capacité',
      requireApproval: 'Approbation requise',
      notificationEmail: 'E-mail de notification',
      conferenceEnabled: 'Lien de réunion en ligne',
      serviceSelectionEnabled: 'Sélection de service',
      paymentEnabled: 'Paiement marketplace',
      blockedDates: 'Dates bloquées',
      holidays: 'Jours fériés',
      agent: 'Agent de réservation',
      sync: 'Sync des horaires',
      minutesUnit: 'min',
      daysUnit: 'jours',
      capacityUnits: {
        booking: 'réservation',
        person: 'personne',
      },
    },
  },
}
