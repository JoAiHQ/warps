import { en } from './en'

export const de: typeof en = {
  book: {
    title: 'Termin buchen',
    pickDate: 'Datum wählen',
    pickSlot: 'Uhrzeit wählen',
    noSlots: 'Keine verfügbaren Termine an diesem Tag. Probiere einen anderen Tag.',
    loadingSlots: 'Verfügbarkeit prüfen...',
    yourDetails: 'Deine Angaben',
    namePlaceholder: 'Dein Name',
    emailPlaceholder: 'Deine E-Mail',
    purposePlaceholder: 'Worum geht es? (optional)',
    notesPlaceholder: 'Noch etwas, das wir wissen sollten? (optional)',
    confirm: 'Termin bestätigen',
    booking: 'Wird gebucht...',
    confirmed: 'Gebucht!',
    confirmedMessage: 'Dein Termin ist bestätigt.',
    backToSlots: 'Zurück',
    today: 'Heute',
    tomorrow: 'Morgen',
    nameRequired: 'Name ist erforderlich.',
    timeZone: 'Zeitzone',
    cancelAppointment: 'Termin absagen',
    cancelling: 'Wird abgesagt...',
    cancelled: 'Termin abgesagt',
    cancelledMessage: 'Dein Termin wurde abgesagt.',
    cancelConfirm: 'Diesen Termin wirklich absagen?',
    cancelConfirmYes: 'Ja, absagen',
    cancelConfirmNo: 'Behalten',
    cancelReasonLabel: 'Grund (optional)',
    cancelReasonPlaceholder: 'Warum sagst du ab?',
    slotsError: 'Verfügbare Zeiten konnten nicht geladen werden. Bitte erneut versuchen.',
    bookingError: 'Dieser Termin konnte nicht gebucht werden. Bitte wähle eine andere Zeit.',
    cancelError: 'Dein Termin konnte nicht abgesagt werden. Bitte erneut versuchen.',
    tryAgain: 'Erneut versuchen',
    copyLink: 'Link teilen',
    copied: 'Kopiert!',
    serviceDuration: 'Min.',
    bookService: 'Buchen',
    noSpecificService: 'Kein bestimmter Service',
  },
  confirm: {
    confirmed: 'Bestätigt!',
    confirmedMessage: 'Dein Termin ist bestätigt.',
    notFound: 'Termin nicht gefunden',
    notFoundMessage: 'Dieser Termin wurde möglicherweise entfernt oder der Link ist ungültig.',
    cancelAppointment: 'Termin absagen',
    cancelling: 'Wird abgesagt...',
    cancelled: 'Termin abgesagt',
    cancelledMessage: 'Dieser Termin wurde abgesagt.',
    cancelConfirm: 'Diesen Termin wirklich absagen?',
    cancelConfirmYes: 'Ja, absagen',
    cancelConfirmNo: 'Behalten',
    cancelReasonLabel: 'Grund (optional)',
    cancelReasonPlaceholder: 'Warum sagst du ab?',
    cancelError: 'Konnte nicht abgesagt werden. Bitte erneut versuchen.',
    joinTeams: 'Per Microsoft Teams beitreten',
    joinGoogleMeet: 'Per Google Meet beitreten',
    joinZoom: 'Per Zoom beitreten',
    joinMeeting: 'Meeting beitreten',
  },
  configure: {
    title: 'Termineinstellungen',
    officeHours: 'Bürozeiten',
    closed: 'Geschlossen',
    bookingRules: 'Buchungsregeln',
    save: 'Einstellungen speichern',
    saved: 'Gespeichert',
    saving: 'Speichern...',
    fields: {
      slotInterval: 'Slot-Intervall (Min)',
      bufferAfter: 'Pufferzeit (Min)',
      minNotice: 'Mindestvorlaufzeit (Min)',
      maxDaysAhead: 'Maximale Vorlaufzeit (Tage)',
      conferenceEnabled: 'Online-Meeting-Link',
      serviceSelectionEnabled: 'Serviceauswahl erlauben',
      paymentEnabled: 'Marketplace-Zahlung erforderlich',
      serviceDurationUnit: 'Min.',
    },
    days: {
      monday: 'Mo',
      tuesday: 'Di',
      wednesday: 'Mi',
      thursday: 'Do',
      friday: 'Fr',
      saturday: 'Sa',
      sunday: 'So',
    },
  },
  policyGet: {
    title: 'Terminrichtlinie',
    empty: 'Noch keine Terminrichtlinie eingerichtet.',
    active: 'Aktiv',
    inactive: 'Inaktiv',
    officeHours: 'Bürozeiten',
    noHours: 'Keine Bürozeiten hinterlegt.',
    bookingRules: 'Buchungsregeln',
    details: 'Details',
    editSettings: 'Einstellungen bearbeiten',
    yes: 'Ja',
    no: 'Nein',
    syncEnabled: 'Mit Öffnungszeiten synchronisiert',
    syncDisabled: 'Nicht synchronisiert',
    syncError: 'Sync-Fehler',
    fields: {
      slotInterval: 'Slot-Intervall',
      bufferAfter: 'Pufferzeit',
      minNotice: 'Mindestvorlaufzeit',
      maxDaysAhead: 'Max. Tage im Voraus',
      capacity: 'Kapazität',
      requireApproval: 'Freigabe erforderlich',
      notificationEmail: 'Benachrichtigungs-E-Mail',
      conferenceEnabled: 'Online-Meeting-Link',
      serviceSelectionEnabled: 'Serviceauswahl',
      paymentEnabled: 'Marketplace-Zahlung',
      blockedDates: 'Gesperrte Tage',
      holidays: 'Feiertage',
      agent: 'Buchungsagent',
      sync: 'Öffnungszeiten-Sync',
      minutesUnit: 'Min.',
      daysUnit: 'Tage',
      capacityUnits: {
        booking: 'Buchung',
        person: 'Person',
      },
    },
  },
}
